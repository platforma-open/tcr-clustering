...
gliph2 <- function(cdr3_sequences,
  ...
  if(local_similarities == TRUE){
    ...
    # BEFORE: motifs_df <- dplyr::left_join(x = motifs_df, y = ref_motifs_df, by = "motif")
    # PATCHED:
    motifs_df$motif <- base::as.character(motifs_df$motif); ref_motifs_df$motif <- base::as.character(ref_motifs_df$motif); motifs_df <- dplyr::left_join(x = motifs_df, y = ref_motifs_df, by = "motif")
    motifs_df[base::is.na(motifs_df)] <- 0
  ...
  if(global_similarities == TRUE){
    ...
    if(base::length(unq_smaple_struct) > 0){
      ...
      if(base::nrow(exp_reference_seqs) == 0){
        # BEFORE: ref_stats <- base::data.frame(1, 1)[-1,]
        # PATCHED:
        ref_stats <- base::data.frame(tag = base::character(0), Freq = base::integer(0), stringsAsFactors = FALSE)
      } else {
        ref_stats <- base::data.frame(base::table(exp_reference_seqs$tag), stringsAsFactors = FALSE)
      }
      base::colnames(ref_stats) <- c("tag", "Freq")
      # BEFORE: sample_stats <- dplyr::left_join(sample_stats, ref_stats, by = "tag")
      # PATCHED:
      sample_stats$tag <- base::as.character(sample_stats$tag); ref_stats$tag <- base::as.character(ref_stats$tag); sample_stats <- dplyr::left_join(sample_stats, ref_stats, by = "tag")
